...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/08 12:25:57 by bschwitz          #+#    #+#             */
/*   Updated: 2022/02/09 15:20:08 by bschwitz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*check si il y a que des murs autours */
{
	int	x;
	int	y;
...
/* check la map (0,1,P,C,E) */
{
	...
	while (data->map->map && data->map->map[y])
	{
		...
		data->map->beer += ft_count_c(data->map->map[y], 'C');
		y++;
	}
	if (player != 1)
...
/* map .ber en tableau 2 dimensions char* */
{
	int		fd;
	int		i;
	int		bytes;
